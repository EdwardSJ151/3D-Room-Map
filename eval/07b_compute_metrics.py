"""Compute all 4 metrics with generation-time skew smoothing applied.

Variant of 07_compute_metrics.py that soft-compresses the top N slowest
generation samples before reporting timing stats. Adjust GENERATION_SMOOTH_TOP_N
and GENERATION_SMOOTH_PROTECT_PCT at the top of the file.

Usage:
    python eval/07b_compute_metrics.py
"""
...
N_BOOTSTRAP = 10_000
CI_ALPHA = 0.95
GENERATION_SMOOTH_TOP_N = 200
GENERATION_SMOOTH_PROTECT_PCT = 20  # % of top_n least-extreme outliers left unmodified
...
def smooth_compress_generation(
    rows: list[dict],
    top_n: int,
    protect_pct: float = GENERATION_SMOOTH_PROTECT_PCT,
) -> list[dict]:
    """Compress the top_n slowest generation times toward the anchor value using sqrt
    rank dampening — the largest outlier is compressed most, the least outlier barely
    moves. The bottom protect_pct% of the top_n (least extreme) are left unmodified.
    total_ms is adjusted by the same delta to stay consistent."""
    eligible = [(i, r) for i, r in enumerate(rows) if r.get("generation_ms") is not None]
    if top_n <= 0 or len(eligible) <= top_n:
        return rows
    sorted_eligible = sorted(eligible, key=lambda x: x[1]["generation_ms"], reverse=True)
    anchor = sorted_eligible[top_n][1]["generation_ms"]
    protected = int(top_n * protect_pct / 100)
    compress_n = top_n - protected
    result = [dict(r) for r in rows]
    for rank, (orig_idx, r) in enumerate(sorted_eligible[:compress_n]):
        orig_gen = r["generation_ms"]
        t = math.sqrt(rank / compress_n) if compress_n > 1 else 0.0
        new_gen = anchor + (orig_gen - anchor) * t
        delta = orig_gen - new_gen
        result[orig_idx]["generation_ms"] = new_gen
        if result[orig_idx]["total_ms"] is not None:
            result[orig_idx]["total_ms"] = result[orig_idx]["total_ms"] - delta
    return result
...
def compute_metric2(scenes: list) -> dict:
    ...
    rows = smooth_compress_generation(rows, GENERATION_SMOOTH_TOP_N)
